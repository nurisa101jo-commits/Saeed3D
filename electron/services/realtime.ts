...
export class OpenAIRealtime{
  ...
  private connect(){if(this.stopped||!this.key)return;this.callbacks.state?.('connecting');let socket:any;try{socket=new WebSocket('wss://api.openai.com/v1/realtime?model='+encodeURIComponent(this.model),{headers:{Authorization:'Bearer '+this.key}})}catch(e){this.fail(String(e));return}this.ws=socket;socket.on('open',()=>{if(this.ws!==socket)return;this.retryMs=3000;this.send({type:'session.update',session:{type:'realtime',model:this.model,output_modalities:['audio'],audio:{input:{format:{type:'audio/pcm',rate:24000},transcription:{model:'gpt-transcribe'},turn_detection:{type:'semantic_vad',eagerness:'high',interrupt_response:true,create_response:true}},output:{format:{type:'audio/pcm',rate:24000},voice:'marin'}},instructions:this.instructions}});this.callbacks.state?.('connected')});socket.on('message',(raw:any)=>{try{this.callbacks.event?.(JSON.parse(raw.toString()))}catch{}});socket.on('error',(e:any)=>this.fail(String(e?.message||e)));socket.on('close',()=>{if(this.ws===socket)this.ws=null;if(!this.stopped){this.callbacks.state?.('disconnected','Realtime connection closed');this.scheduleRetry()}})}
