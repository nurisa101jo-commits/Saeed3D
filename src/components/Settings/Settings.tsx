...
type S={llmProvider:string;llm:any;sttModel:string;ttsProvider:string;tts:any;language:string;alwaysListening:boolean};
const defaults:S={llmProvider:'openai',llm:{model:'gpt-5'},sttModel:'gpt-4o-mini-transcribe',ttsProvider:'openai',tts:{model:'gpt-4o-mini-tts',voice:'alloy'},language:'en',alwaysListening:true};
...
export function Settings(){
 ...
 const ttsFields=s.ttsProvider==='azure'?(
  <><label>Azure region<input value={s.tts.region||''} onChange={e=>setS({...s,tts:{...s.tts,region:e.target.value}})}/></label><label>Azure voice<input value={s.tts.voice||''} onChange={e=>setS({...s,tts:{...s.tts,voice:e.target.value}})}/></label><label>Azure key<input type="password" placeholder={has.azure?'Saved securely':'Enter Azure Speech key'} value={ttsKey} onChange={e=>setTtsKey(e.target.value)}/></label></>
 ) : (
 ...
 <label>LLM API key<input type="password" placeholder={has[s.llmProvider]?'Saved securely':'Enter API key'} value={llmKey} onChange={e=>setLlmKey(e.target.value)}/></label>
 <div className="settingActions"><button onClick={save}>Save</button><button onClick={testLLM}>Test LLM</button></div>
 <div className="status">LLM: {status.llm?.status|| (has[s.llmProvider]?'saved':'not configured')} {status.llm?.message||''}</div>
 <hr/><h4>Speech</h4>
 <label>STT model (OpenAI)<input value={s.sttModel} onChange={e=>setS({...s,sttModel:e.target.value})}/></label>
 <label>TTS provider<select value={s.ttsProvider} onChange={e=>setS({...s,ttsProvider:e.target.value})}><option value="openai">OpenAI TTS</option><option value="azure">Azure Speech</option></select></label>
 {ttsFields}
 <div className="settingActions"><button onClick={testTTS}>Test TTS</button></div>
 <div className="status">TTS: {status.tts?.status||'not tested'} {status.tts?.message||''}</div>
 <label>Language<input value={s.language} onChange={e=>setS({...s,language:e.target.value})}/></label>
 <label><input type="checkbox" checked={s.alwaysListening} onChange={e=>setS({...s,alwaysListening:e.target.checked})}/> Always listen</label>
 <button onClick={async()=>{await save();setOpen(false)}}>Save & close</button>
 </div>}</>
}