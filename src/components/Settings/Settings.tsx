...
const defaults:S={
  ...
  language:'en',alwaysListening:true,brainMode:'auto',showSpeechText:false,speakResponses:true
};
...
export function Settings(){
  ...
  useEffect(()=>{
    window.electronAPI.getSettings().then(x=>{
      setS({...defaults,...x.config,llm:{...defaults.llm,...x.config?.llm},tts:{...defaults.tts,...x.config?.tts}});
      setHas(x.hasSecrets||{});
    });
  ...
  async function save(){
    ...
    await window.electronAPI.saveSettings({...s,secrets});
    setLlmKey('');setSttKey('');setTtsKey('');
    const x=await window.electronAPI.getSettings();setHas(x.hasSecrets||{});
    setStatus((v:any)=>({...v,save:'Settings saved'}));
  }
  ...
  async function disableExternal(){
    await window.electronAPI.clearApiKeys('all');
    const next={...s,brainMode:'local' as const,sttProvider:'local' as const,ttsProvider:'local' as const};
    setS(next);await window.electronAPI.saveSettings(next);
    const x=await window.electronAPI.getSettings();setHas(x.hasSecrets||{});
  ...
  return <><button className="settingsButton" onClick={()=>setOpen(!open)}>⚙ Settings</button>{open&&<div className="settingsPanel">
    ...
    <div className="settingActions"><button onClick={testSTT}>Test STT</button></div>
    <div className="status">STT: {status.stt?.status||'not tested'} {status.stt?.message||''}</div>
    ...
    <div className="settingActions"><button onClick={async()=>{const r=await window.electronAPI.changeAvatarModel();if(r.changed)setStatus((x:any)=>({...x,character:'Character changed'}));}}>Change Character (.vrm)</button></div>
    {status.character&&<div className="status">{status.character}</div>}
    <div className="settingActions"><button onClick={async()=>{await save();await window.electronAPI.checkForUpdates()}}>Check for updates</button><button onClick={async()=>{await save();setOpen(false)}}>Save & close</button></div>
    {status.save&&<div className="status">{status.save}</div>}
  </div>}</>;
}
